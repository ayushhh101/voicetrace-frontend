                ...
                <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-6">
                        <StatCard
                            ...
                            color="red" // Force it to use the red color you defined in StatCard
                        />

                        <StatCard
                            label="Log"
                            value={(data.people || []).filter(p => p.pendingAmount > 0).length}
                            isCurrency={false}
                            color="red"
                        />

                        {/* Let's make the Status box a StatCard too for consistency */}
                        <StatCard
                            label="Status"
                    ...
                    <div className="space-y-3">
                        {data.people.length > 0 ? (
                            data.people.map((person, idx) => {
                                // Determine status colors based on the amount
                                ...
                                return (
                                    <DebtPerson
                                        ...
                                        isCleared={isZero}
                                        // Logic for colors: 
                                        // If negative (You owe): Blue
                                        // If positive (They owe): Alternate Rose/Orange
                                        color={
                                            isNegative
                                                ? "bg-blue-50 text-blue-600"
                                                : (idx % 2 === 0 ? "bg-rose-50 text-rose-600" : "bg-orange-50 text-orange-600")
                                        }
                                        initialBg={
                                            isNegative
                                                ? "bg-blue-100 text-blue-700"
                                                : (idx % 2 === 0 ? "bg-rose-100 text-rose-700" : "bg-orange-100 text-orange-700")
                                        }
                                    />
                                );
                            })
        ...
        <div className="flex items-center gap-4">
            <div className="text-right mr-2">
                {isCleared ? (
                    ...
                    <>
                        <p className="text-lg font-black text-[#9F1239]">₹{Math.abs(amount)}</p>
                        <p className="text-[9px] font-bold text-slate-300 uppercase tracking-tighter">
                            {amount > 0 ? 'pending' : 'to pay'}
                        </p>
                    </>
                )}
            </div>
